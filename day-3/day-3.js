...
const lineResults = lines.map((line, lineIndex) => {
    ...
    const adjacentNumbers = matches.filter(match => {
        const startIndex = match.index - 1;
        const endIndex = match.index + match[0].length;
        const previousLine = lines[lineIndex - 1];
        const nextLine = lines[lineIndex + 1];

        const charsToCheck = `${line.charAt(startIndex)}${line.charAt(endIndex)}${previousLine ? previousLine.substring(startIndex, endIndex + 1) : ''}${nextLine ? nextLine.substring(startIndex, endIndex + 1) : ''}`;
        // debugger;

        // console.log('charsToCheck', charsToCheck, match);
        
        return charsToCheck.match(symbolsRegex);
    });

    const lineTotal = adjacentNumbers.reduce((acc, num) => acc += parseInt(num[0]), 0);
